...
├── src/
│   ├── components/
│   │   ├── employee_form.rs      # For adding and updating employees
│   │   ├── employee_list.rs      # To list all employees
│   │   └── mod.rs                # To import the components
│   ├── request.rs                # To handle API requests (GET, POST, PUT, DELETE)
│   ├── main.rs                   # Entry point for the Yew app
│   ├── styles.css                # Any CSS for styling (optional)
│   └── lib.rs                    # Rust entry point for WASM
├── static/                       # Static assets (e.g., images, fonts, etc.)
├── Cargo.toml                    # Your project dependencies and configuration
└── Trunk.toml
